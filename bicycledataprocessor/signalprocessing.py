...
def frame_acceleration(AccelerationX, AccelerationY, AccelerationZ, 
					lam, rollAngle=0.):
	'''Returns the bicycle frame acceleration based on the body
	fixed acceleration taken with the VN-100 and optionally the roll angle
	measurement.

	Parameters
	----------
	AccelerationX : ndarray, shape(n,)
		The body fixed acceleration perpendicular to the headtube and pointing forward.
	AccelerationY : ndarray, shape(n,)
		The body fixed acceleration perpendicular to the headtube and pointing to the
		right of the bicycle.
	AccelerationZ : ndarray, shape(n,)
		The body fixed acceleration aligned with the headtube and pointing downward.
	lam : float
		The steer axis tilt.
	rollAngle : ndarray, shape(n,), optional
		The roll angle of the bicycle frame.
	...
	downward_Acceleration : ndarray, shape(n,)
	...
	lateral_Acceleration = np.cos(rollAngle) * (AccelerationY + AccelerationX * 
			...
			AccelerationZ * np.cos(lam) * np.tan(rollAngle))
